...
<head>
    ...
    <title>@yield('title', 'LakiranjeAutomobila')</title>
...
<body>
    <nav class="w3-sidebar w3-bar-block w3-card w3-top w3-xlarge w3-animate-left" style="display:none;z-index:2;width:40%;min-width:300px" id="mySidebar">
        ...
        @auth
            @if(auth()->user()->role === 'client')
                <a href="{{ route('client.services.create') }}" onclick="w3_close()" class="w3-bar-item w3-button">Zakazivanje</a>
                <a href="{{ route('client.services.index') }}" onclick="w3_close()" class="w3-bar-item w3-button">Moje usluge</a>
            @elseif(auth()->user()->role === 'employee')
                <a href="{{ route('employee.services.index') }}" onclick="w3_close()" class="w3-bar-item w3-button">Usluge za obradu</a>
            @endif
            ...
            <div class="w3-right w3-padding-16">
                @auth
                    @if(in_array(auth()->user()->role, ['admin']))
                        <a href="{{ route('dashboard') }}" class="w3-button">Dashboard</a>
                    @endif
                @else
    ...
    <div class="w3-panel w3-green w3-round w3-padding" style="margin-top: 1rem;">
        {{ session('success') }}
    </div>
